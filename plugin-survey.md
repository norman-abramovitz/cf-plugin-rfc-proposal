...
**Consumer plugin analysis (historical and current):** Git history shows the library solved a real problem — scheduler-plugin used `GetServices()`, `GetService()`, and `GetApp()` for 2.5 years (Feb 2023 – Oct 2025, [`57130bdb`](https://github.com/rabobank/scheduler-plugin/commit/57130bdb)...[`e682b800`](https://github.com/rabobank/scheduler-plugin/commit/e682b800)), and credhub-plugin used `GetService()` for 2+ years ([`e5355478`](https://github.com/rabobank/credhub-plugin/commit/e5355478)...[`7cdaded9`](https://github.com/rabobank/credhub-plugin/commit/7cdaded9)). But the scope was broader than needed: only 3 of 10 reimplemented methods were ever called across all consumers, accessing a combined 5 unique fields. Today, only 1 method (`GetService`) is called by 1 consumer (credhub-plugin), accessing 3 fields. See [full analysis](rfc-draft-plugin-transitional-migration.md#consumer-plugin-analysis-historical-and-current-v2-usage).
