...
10. **Rabobank transitional wrapper caveats:** The Rabobank `cf-plugins` library reveals several issues that the companion package must address: (a) token prefix handling — `config.Token()` handles "bearer " internally, do not strip manually; (b) SSL pass-through — must honor `IsSSLDisabled()` from the host, not hardcode; (c) user agent — should derive from plugin metadata, not hardcode; (d) V2-to-V3 impedance mismatch — reimplementing V2 domain methods via V3 is lossy (IsAdmin always false, single process type, single buildpack, no per-app stats, 11 API calls for GetApp). See [transitional RFC](rfc-draft-plugin-transitional-migration.md#lessons-from-the-rabobank-implementation).
