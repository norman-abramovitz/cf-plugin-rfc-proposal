...
10. ~~**Rabobank transitional wrapper caveats**~~ → **Resolved.** V2-to-V3 data shape differences (IsAdmin, single process, single buildpack, missing stats, 11 API calls) are resolved by the generated wrapper approach — only declared fields are populated with minimum V3 calls. Implementation bugs (token prefix stripping, hardcoded SSL, hardcoded user agent) documented as "bugs to avoid". See [transitional RFC](rfc-draft-plugin-transitional-migration.md#lessons-from-the-rabobank-implementation).
